-- Test HTTP Server - KHÔNG dùng HTTP client (tránh deadlock)
rex.print("🌐 HTTP Server Simple Test")
rex.print("=" .. string.rep("=", 59))
rex.print("\n💡 Server sẽ chạy. Dùng terminal KHÁC để test:")
...
rex.print("\n✅ Server created at " .. server.address)
...
handlers["GET:/"] = function(body)
    return rex.json.stringify({
        message = "Welcome to Rustisaur HTTP Server!",
        version = "0.3.0",
...
handlers["POST:/api/users"] = function(body)
    ...
    return rex.json.stringify({success = true, message = "User created"})
end

handlers["GET:/api/health"] = function(body)
    return rex.json.stringify({status = "healthy"})
end
...
rex.print("📝 Press Ctrl+C to stop\n")
...
while rex.http.server.is_running(server) do
    ...
    rex.os.sleep(10)  -- Ngủ 10ms để không chiếm CPU
end
